...
CCES_Panel.reg <- CCES_Panel %>%
  dummy_cols(select_columns = c("race_12", "partyreg12")) %>%
...
cces.test <- CCES_Panel.reg %>% filter(ideo5_12 < 6, pid3_12 < 4) %>% mutate(ideo5_12 = ifelse(ideo5_12 < 3, 1, ifelse(ideo5_12 == 3, 2, ifelse(ideo5_12 < 3, 3, ideo5_12))), pid3_12 = ifelse(pid3_12 == 2, 3, ifelse(pid3_12 == 3, 2, pid3_12)))

mod1 <- glm(p12switch ~ gender_12 + age12 + race_12_char + pid3_12, binomial, cces.test)

summary(mod1)

mod2 <- glm(p12switch ~ gender_12 + age12 + race_12_char + pid3_12, binomial, CCES_Panel.reg)

summary(mod2)

# creating a person and testing predict
maya <- tibble(gender_12 = 0, age12 = 38, pid3_12 = 1, race_12_char = "other")

predict(mod1, newdata = maya, interval = "confidence")

predict(mod2, newdata = maya, interval = "confidence")
```

```{r visualizing, echo = FALSE}


CCES_Panel %>% filter(p12switch < 2, CC12_302 < 6) %>% group_by(CC12_302) %>% summarize(avg_p12switch = mean(p12switch), emplogit = qlogis(avg_p12switch)) %>% ggplot(aes(CC12_302, emplogit)) + geom_point() + geom_smooth(method = "lm") + labs(x = "Opinion", y = "Empirical logits") + scale_x_continuous(breaks = c(1, 2, 3, 4, 5), label = c("Gotten much better", "Gotten better", "Stayed about the same", "Gotten Worse", "Gotten much worse")) + theme_classic()

# check out stephcheng.shinyapps.
